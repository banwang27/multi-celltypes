...
for (celltype in c('CM','MN','PP','HP','SKM','RPE')){
    deseq2 = read.csv(paste0('/scratch/users/banwang3/celllines/analysis/DESeq2_norm/human/',celltype,'_aligned2human_qvalue_DESeq2.txt'),sep='\t')
    deseq2m = read.csv(paste0('/scratch/users/banwang3/celllines/analysis/DESeq2_norm/merged/',celltype,'_merged_qvalue_DESeq2.txt'),sep='\t')
...
data$celltype=factor(data$celltype,levels=c('MN','CM','HP','PP','SKM','RPE'))
output_path='/scratch/users/banwang3/celllines/scripts/RScripts/Rplots11/'
pdf(paste0(output_path,'MN_set_LFC.pdf'),useDingbats=FALSE,width=4,height=3)
print(max(data$LFC))
print(min(data$LFC))
ggplot(data, aes(factor(celltype), gene)) + 
  geom_tile(aes(fill=LFC),color='black',height=1,width=1)+
  geom_text(aes(label=DE))+
  scale_fill_gradientn(colours = c("dodgerblue4", "white", "indianred3"),limits=c(-2.1,2.1))+
  xlab('')+
  ylab('')+
  theme_bw()
dev.off()
