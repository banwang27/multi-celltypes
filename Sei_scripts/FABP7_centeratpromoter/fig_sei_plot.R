...
plotSEIrawoutput<-function(df1,xaxis,yaxis,idcandidate=c('Neuron.H3K4me3.ID.64664','iPSC.H3K4me3.ID.75085'),xlim1=0.2,xlim2=1,ylim1=0.2,ylim2=1){
    ...
    p<-p+
        ...
        #geom_label_repel(data=pointlabel,aes(x=x,y=y,label=id),box.padding   = 0.35, point.padding = 0.5,segment.color = 'grey50',size=3,min.segment.length = 0) 
...
plotSEIhistogram<-function(df1,xaxis,pointlabel,xlim1=0.2,xlim2=1,ylim1=0,ylim2=1){
    ...
    p<-p+
        ...
        #geom_label_repel(data=toplot,aes(x=score,y=y,label=id),box.padding   = 0.35, point.padding = 0.5,segment.color = 'grey50',size=3,min.segment.length = 0) 
...
df_hg38= read.csv(paste0('/scratch/users/banwang3/references/outputs/FABP7_centeratpromoter/sei_hg38/sei/','hg38_centeratpeak_sei_pred_lfc.txt'),sep='\t')
newdf=df_hg38[,c(30,1:29)]
newdf1=df[,c(31:33,3:30)]
newdf = cbind(newdf,newdf1)
newdf1 = df[,c('hum_chp_fastalfc','hum_indelhum_fastalfc','hum_humdel1G_fastalfc','hum_humadd3bp_fastalfc')]
newdf = cbind(newdf,newdf1)
idx = grep('humanbased_humchplfc_withSNP',colnames(df_hg38))   
newdf = cbind(newdf,df_hg38[,idx])
tmpnames=colnames(newdf)
newdf = cbind(newdf,df_hg38[,'hum_hum2chp_fastalfc'])
colnames(newdf)=c(tmpnames,'hum_hum2chp_fastalfc')
idx = grep('chimpbased_humchplfc_withSNP',colnames(df))
newdf = cbind(newdf,df[,idx])
tmpnames=colnames(newdf)
newdf=cbind(newdf,df[,'chp2hum_chp_fastalfc'])
colnames(newdf)=c(tmpnames,'chp2hum_chp_fastalfc')
write.table(newdf,file='FABP7_centeratpromoter_summary_SEI.txt',sep='\t',quote=F,row.names=F,col.names=T)
############
...
ids = c("hum_chp_fastalfc","hum_indelhum_fastalfc","humanbased_humchplfc_withSNP1")
...
ids = c('hum_indelhum_fastalfc','hum_humdel1G_fastalfc','hum_humadd3bp_fastalfc','humanbased_humchplfc_withSNP1','humanbased_humchplfc_withSNP15','humanbased_humchplfc_withSNP16','humanbased_humchplfc_withSNP26')
for (id in ids){
    data = data.frame(id=id,x=df1[,id],y=df1$hum_chp_fastalfc)
    tmp = getCorString(df1,id,'hum_chp_fastalfc')
    print(id)
    print(tmp)
    print(sqrt(mean((data$x-data$y)**2)))
}
for (id in ids){
...
constructBoxplot<-function(df1,species,includeSNPall=T){
    ...
    df2 = rbind(df2,sei)
    sei=data.frame(lfc=df1[,'hum_indelhum_fastalfc'],SNP='Insertion\n+Deletion',type='Combined candidate causal variants effect')
    df2 = rbind(df2,sei)
    sei=data.frame(lfc = df1[,allSNPflipid],SNP='SNPall',type='Net effect')
    df2 = rbind(df2,sei)
    df2$SNP = factor(df2$SNP,levels=c('Chimp','SNPall','Insertion\n+Deletion',paste0('SNV',seq(1,8)),'Insertion','SNV9','SNV10','Deletion',paste0('SNV',seq(11,nSNP))))
    #df2$type = factor(df2$type,levels=c('Net effect','SNP effect'))
    df2$type = factor(df2$type,levels=c('Net effect','Combined candidate causal variants effect','Individual candidate causal variant effect','SNV effect'))
    if (!includeSNPall){
