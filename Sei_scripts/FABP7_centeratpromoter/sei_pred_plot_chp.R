...
library(ggplot2)
library(patchwork)
input_path='/scratch/users/banwang3/references/outputs/FABP7_centeratpromoter/sei_pantro6/sei/'
...
idx = which(df[,paste0('chimpfasta')]>=0.5| df[,paste0('chimp2humanfasta')]>=0.5)
...
sei=data.frame(lfc = df1[,'chp2hum_chp_fastalfc'],SNP='chp2humFasta',type='fasta')
human = rbind(human,sei)
...
human$SNP = factor(human$SNP,levels=c('chp2humFasta',paste0('SNP',seq(1,nSNP))))
...
p2<-ggplot(pearsonr,aes(x = SNP,y=pearsonr))+ 
    ...
    geom_bar(stat = "identity",color="black",fill="white",width=0.5)+                           
     ...
     theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),panel.grid.major=element_blank(),panel.grid.minor=element_blank())
p<-p1/p2
pdf(paste0(output_path,'FABP7_pantro6_centeratpeak_sei_pred_chp2hum_vs_chp.pdf'),useDingbats=FALSE,width=15,height=8)
print(p)
dev.off()
