...
library(ggplot2)
library(patchwork)
input_path='/scratch/users/banwang3/references/outputs/FABP7/sei/'
output_path='/scratch/users/banwang3/references/scripts/seifigures/'

df = read.csv(paste0(input_path,'hg38_centeratpeak_sei_pred_lfc.txt'),sep='\t')
#idx = which(df[,paste0('humanfasta')]>=0.5| df[,paste0('human2chimpfasta')]>=0.5)
...
for (i in seq(1,nSNP)){ 
    ...
    sei = data.frame(lfc = df1[,paste0('humanbased_humchplfc_withSNP',i)],SNP = paste0('SNP',i),type='humanbasedSNP')
    
    corresult=cor.test(df1[,paste0('fastalfc')],df1[,paste0('humanbased_humchplfc_withSNP',i)])
    #corresult=cor.test(df1[,paste0('hum_hum2chp_fastalfc')],df1[,paste0('humanbased_humchplfc_withSNP',i)])
...
#sei=data.frame(lfc = df1[,'hum_hum2chp_fastalfc'],SNP='hum2chpFasta',type='fasta')
human = rbind(human,sei)
...
#human$SNP = factor(human$SNP,levels=c('hum2chpFasta',paste0('SNP',seq(1,nSNP))))
...
p2<-ggplot(pearsonr,aes(x = SNP,y=pearsonr))+ 
    ...
    geom_bar(stat = "identity",color="black",fill="white",width=0.5)+                           
     ...
     theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),panel.grid.major=element_blank(),panel.grid.minor=element_blank())
p<-p1/p2
pdf(paste0(output_path,'FABP7_hg38_centeratpeak_sei_pred_hum_vs_chp.pdf'),useDingbats=FALSE,width=15,height=8)
print(p)
dev.off()
