...
for (i in seq(1,nSNP)){ 
    ...
    df1 = df[idx,] 
    if (nrow(df1)<=2){
        fastalfc=data.frame(lfc=0,SNP=paste0('SNP',i),type='fasta')
        sei=data.frame(lfc=0,SNP = paste0('SNP',i),type='chimpbasedSNP')
        pearsonr=c(pearsonr,0)
        pval=c(pval,1)
    }else{
        fastalfc=data.frame(lfc=df1[,paste0('fastalfc_centeratSNP',i)],SNP=paste0('SNP',i),type='fasta')
        sei = data.frame(lfc = df1[,paste0('chimpbased_humchplfc_centeratSNP',i)],SNP = paste0('SNP',i),type='chimpbasedSNP')
        corresult=cor.test(df1[,paste0('fastalfc_centeratSNP',i)],df1[,paste0('chimpbased_humchplfc_centeratSNP',i)])
