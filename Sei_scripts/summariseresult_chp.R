...
df = data.frame(t(df))
nSNP=ncol(df)/3
colnames(df)=c(paste0('humanfasta_centeratSNP',seq(1:nSNP)),paste0('chimpfasta_centeratSNP',seq(1:nSNP)),paste0('chimpfasta_withSNP',seq(1:nSNP)))
df = df[-c(1,2),]
...
for (i in seq(1,nSNP)){
    ref = df[,i]
    alt = df[,i+nSNP]
    altSNP = df[,i+nSNP+nSNP]
    fastalfc = cbind(fastalfc,log2(ref/alt))
    SNPlfc = cbind(SNPlfc,log2(altSNP/alt))
}
tmpnames=colnames(df)
df = cbind(df,fastalfc,SNPlfc)   
colnames(df)=c(tmpnames,paste0('fastalfc_centeratSNP',seq(1:nSNP)),paste0('chimpbased_humchplfc_centeratSNP',seq(1:nSNP)))
