...
result$binlab<-NA
result[result$expressionbin==1,'binlab']<-paste0('Genes in ','0-20%', ' expression level')
result[result$expressionbin==2,'binlab']<-paste0('Genes in ','20-40%',' expression level')
result[result$expressionbin==3,'binlab']<-paste0('Genes in ','40-60%',' expression level') 
result[result$expressionbin==4,'binlab']<-paste0('Genes in ','60-80%',' expression level')  
result[result$expressionbin==5,'binlab']<-paste0('Genes in ','80-100%',' expression level')   
result$binlab=factor(result$binlab,levels=paste0('Genes in ',c('0-20%','20-40%','40-60%','60-80%','80-100%'),' expression level'))
...
pdf(paste0(output_path,'OR_test_expressionlevel_',SKM,'.pdf'), useDingbats=FALSE,width=9,height=7)
ggplot(data=result,aes(y=celltype)) +
    geom_point(aes(x=OR))+
    geom_segment(aes(x=lower,xend=upper,yend=celltype))+
    ylab('Cell Type') +
    xlab('Odds Ratio')+
    facet_wrap(~binlab,scales="free") +
...
pdf(paste0(output_path,'OR_test_expressionlevel_',SKM,'_v2.pdf'), useDingbats=FALSE,width=8,height=5)
