...
for (celltype in celltypes){
    dfopen = read.csv(paste0(input_path,celltype,'_ATAC_Filtered_Down_fixed.txt'),sep='\t')
...
for (keyword in c('pval','FDR','lfc')){
    ...
    pdf(paste0(output_path,'ATAC_OR_test_Downsampled_cutoff_',keyword,'.pdf'), useDingbats=FALSE,width=4,height=9)
    print(p)
    dev.off()
}   
