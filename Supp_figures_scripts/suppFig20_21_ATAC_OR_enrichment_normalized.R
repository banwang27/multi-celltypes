...
input_path='/scratch/users/banwang3/celllines/scripts/RScripts/figscripts/suppfigscripts/ATAC_counttable/normalized/Normalized_binom/'
output_path='/scratch/users/banwang3/celllines/scripts/RScripts/Rplots/'
result=c()
pvalcutoffs=c(0.1,0.05,0.01)
for (celltype in celltypes){
    dfopen = read.csv(paste0(input_path,celltype,'_Humreffed_NotDown_Filtered_Normed_Binom.txt'),sep='\t')
    #dfopen = dfopen[,c('Peak.Humreffed','l2fc_Humr','Human.binom_pval','Human.FDR')]    
    dfopen = dfopen[,c('Peak.Humreffed','Binomial.p.value')]
...
for (keyword in c('pval')){
    ...
    pdf(paste0(output_path,'ATAC_OR_test_cutoff_',keyword,'_normalized.pdf'), useDingbats=FALSE,width=4,height=9)
    print(p)
    dev.off()
}
