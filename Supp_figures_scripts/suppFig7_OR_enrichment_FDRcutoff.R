...
for (cutoff in c(0.1,0.05,0.01)){
    ...
    ORtest=data.frame(celltype=celltypes,OR=OR,pvals=pvals,lower=lower,upper=upper,FDRcutoff=cutoff)
    result=rbind(result,ORtest)
}
result$plab <- ifelse(result$pvals<0.001,'***',ifelse(result$pvals<0.01,'**',ifelse(result$pvals<0.05,'*','')))
result$FDRlab<-paste0('FDR <= ',result$FDRcutoff) 
pdf(paste0(output_path,'OR_test_FDRcutoff_',SKM,'.pdf'), useDingbats=FALSE,width=4,height=9)
ggplot(data=result,aes(y=celltype)) +
    geom_point(aes(x=OR))+
    geom_segment(aes(x=lower,xend=upper,yend=celltype))+
    ylab('Cell Type') +
    xlab('Odds Ratio')+
    facet_wrap(~FDRlab,nrow=length(unique(result$FDRlab))) +
